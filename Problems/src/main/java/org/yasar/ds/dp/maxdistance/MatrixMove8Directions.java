...
public class MatrixMove8Directions {
	// tool matrices to recur to 8 adjacent cells. 
	...
	public static void main(String[] args) {
		...
		MatrixMove8Directions prob= new MatrixMove8Directions();
	...
	boolean isValidPosition(char[][] arr, int i, int j) {
	...
	int findLongestConsecutivePath(char[][] arr, int i, int j, int prevI, int prevJ) {
		...
		if(!isValidPosition(arr, i, j)) {
			return 0;
		}
		
		//comparing current cell with previous-cell for differeence of 1
		if(prevI!= Integer.MIN_VALUE && prevJ!= Integer.MIN_VALUE && arr[i][j]-arr[prevI][prevJ]!= 1) {
			return 0;
		}
		int res= 0;
		
		// getting max-path by traversing all 8 directions from a point
		for(int move = 0 ; move < x.length ; move++)
			res = Math.max(res, 1 + findLongestConsecutivePath(arr, i + x[move], j + y[move], i, j));
		/*
